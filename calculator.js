...
const MODAL_CONTENT = {
  announcement: {
    title: '更新公告',
    html: `
      <h3>特别适配提示 <span class="modal-date">· 2026-08-11</span></h3>
      <ul>
        <li>对于特别适配的特性和技能，如离心舞者的「和弦共振」，现在会在精灵面板顶部出现图标以提示其存在。</li>
        <li>优化了页面选取，现在不会意外选取到控件的文字了（特别是对于移动端）。</li>
        <li>修复了星陨滑条底部读数不准的问题。</li>
      </ul>
      <hr>
      <h3>数据管理 / 页脚 <span class="modal-date">· 2026-08-06</span></h3>
      <ul>
        <li>新增数据管理窗口（在副标题），可开关记忆功能，清除、导出、导入数据。</li>
        <li>新增页脚区域，加入版权声明，BWIKI 链接迁移至页脚。</li>
      </ul>
      <hr>
      ...
      <ul>
        <li>现在可以通过技能列表底部的自定义技能槽让精灵使用自己无法学习的技能，模拟「借用 / 取念 / 复写 / 过山车 / 奇异」的情况。</li>
        <li>该功能同时也支持记忆和挑战模式。</li>
      </ul>
      <hr>
      ...
      <ul>
        <li><strong>挑战模式上线</strong>：这是一个考验你对星陨斩杀线的把握，辅助记忆斩杀线的小游戏。</li>
        <li>基本流程：挑战设置 → 开始挑战 → 自动出题 → 拖动星陨层数 → 提交答案 → 评分 → 下一道题 / 结算。</li>
        <li>双方精灵池支持「固定 / 常见 / 全部」；属性配置与技能选择支持「记忆 / 随机」；防御技能按相同减伤率兜底匹配。</li>
      ...
      <ul>
        <li><strong>「应对状态」机制上线</strong>：防御方新增「聚能」选项；激活后，<strong>所有应对状态技能</strong>会按各自规则改变威力或连击数，覆盖铁蒺藜、龙卷风、追打、炙热波动、虫击、突袭、暗突袭、爆冲、技巧打击、无影脚、偷袭、散手、连续爪击、滚雪球、吹炎、地陷、闪燃、灾厄。</li>
  ...
  guide: {
    ...
    html: `
      ...
      <h3>特别适配</h3>
      <ul>
        <li>精灵特性，包括：仪式巨像家族、离心舞者家族、岚鸟家族、凡鹰家族、机幕方舟家族、画间沉铁兽、独角兽家族、风滚暮虫家族、蒲公英家族、墨鱿士家族、鸭吉吉家族。</li>
      ...
      <h3>数据来源</h3>
      <ul>
        <li>精灵与技能数据来自 <a href="https://wiki.biligame.com/nrc" target="_blank" rel="noopener noreferrer">BWIKI</a>。</li>
        <li>数据更新于 2026-09-10。</li>
      </ul>
